...
void setup()
  { //init comunications 
    ...
    digitalWrite(reset,HIGH);
     ...
     digitalWrite(indikator,ledState);
...
void loop()
 ...
 //Serial.println(SholatNow);
    ...
    runningTextOut(drawTextOut(),75,4);                             // addr: 2 show Masjid Name
...
///    dwMrq(drawDayDate()   ,75,1,3);                             // addr: 3 show Hijriah date
   // dwMrq(msgPuasa(hd_puasa,ty_puasa),75,0,5); 
...
//    if(RunFinish==101) {RunSel = 101; RunFinish =0; reset_x=1;}       //after Iqomah(anim 101) set Message Sholah (anim 102)   
...
void check_azzan()
  ...
  { 
    //SholatNow  = -1;
     for(int i=0; i <=7; i++)
       {
        if(i!=1 and i!=2 and i!=3 and i!=4 and i!=5 and i!=6 and i!=7)  // bukan terbit dan bukan dhuha
          { //Serial.println(i);
            if(floatnow >= sholatT[i])
              {
               // SholatNow = i;
